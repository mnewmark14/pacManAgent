...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeActionFromValues(self, state):
        ...
        for posAction in posActions:
            val = self.computeQValueFromValues(state, posAction)
...
class AsynchronousValueIterationAgent(ValueIterationAgent):
    ...
    def runValueIteration(self):
        ...
        for i in range(self.iterations):
            states = self.mdp.getStates()
            curState = states[i % len(states)]
            if not self.mdp.isTerminal(curState):
                bestAction = self.getPolicy(curState)
                qValue = self.getQValue(curState, bestAction)
                self.values[curState] = qValue
...
class PrioritizedSweepingValueIterationAgent(AsynchronousValueIterationAgent):
    ...
    def runValueIteration(self):
        ...
        for singState in states:
            for action in self.mdp.getPossibleActions(singState):
                for posState, prob in self.mdp.getTransitionStatesAndProbs(singState, action):
                    predecessors[posState].add(singState)
